...
save_name = 'data_skippy_cifar100_big_one'
objective = obj_func('./all_cnn_bi_skippy_cifar100.py',save_name=save_name)
...
lr_rate = ContinuousSpace([1e-4, 1.0e-1], 'lr')        # learning rate#CHRIS tweaked
...
opt = mipego(search_space, objective, time_model, loss_model, ftarget=None,
                 ...
                 available_gpus=available_gpus, bi=True, save_name=save_name,ref_time=None,ref_loss=None,ignore_gpu=ignore_gpu,eval_epochs=eval_epochs,data_augmentation=False)
...
#CHRIS restart code
if False:
    with open('data_skippy_cifar100_big_one_intermediate.json') as f:
        for line in f:
