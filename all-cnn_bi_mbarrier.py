...
def CNN_conf(cfg):
    ...
    #        entry_no -= 1
    with open("mbarrier_Temp_result.txt", "w") as text_file:
        ...
        for x in cfg:
            if isinstance(cfg[x],float):
                print("r", file=text_file)
            elif isinstance(cfg[x],int):
                print("i", file=text_file)
            else:
                print("d",file=text_file)
    #tuple = deap.benchmarks.schaffer_mo(vector)
    call(["./MI_test_functions/mbarrier/mbarrier","mbarrier_Temp_result.txt","mbarrier_Temp_result_out.txt"])
    lines = [line.rstrip('\n') for line in open("mbarrier_Temp_result_out.txt")]
    print("lines:")
    print(lines)
