    ...
    def end_skip(self,layer,filters,kernel,regulizer,act):
        ...
        while j < len(self.skip_connections):
            if self.skip_connections[j][1] <= 0:
                ...
                j += 1
        if K.int_shape(layer)[3] > filters:
...
def CNN_conf(cfg,epochs=1,test=False,gpu_no=0,verbose=0,save_name='skippy_test_train_hist'):
    ...
    for i in range(cfg['stack_0']):
        layer = skip_manager.connect_skip(layer,cfg['filters_0'],cfg['k_0'],cfg['l2'],cfg['activation'])
        layer = Conv2D(cfg['filters_0'], (cfg['k_0'], cfg['k_0']), padding='same', kernel_regularizer=l2(cfg['l2']), bias_regularizer=l2(cfg['l2']))(layer)
        layer = Activation(cfg['activation'])(layer)
    if (cfg['stack_0']>0):
        layer = skip_manager.end_skip(layer,cfg['filters_0'],cfg['k_0'],cfg['l2'],cfg['activation'])
        #maxpooling as cnn
    ...
    for i in range(cfg['stack_1']):
        layer = skip_manager.connect_skip(layer,cfg['filters_2'],cfg['k_2'],cfg['l2'],cfg['activation'])
        layer = Conv2D(cfg['filters_2'], (cfg['k_2'], cfg['k_2']), padding='same', kernel_regularizer=l2(cfg['l2']), bias_regularizer=l2(cfg['l2']))(layer)
        layer = Activation(cfg['activation'])(layer)
    if (cfg['stack_1']>0):
        layer = skip_manager.end_skip(layer,cfg['filters_2'],cfg['k_2'],cfg['l2'],cfg['activation'])
        if not (cfg['max_pooling']):
    ...
    for i in range(cfg['stack_2']):
        layer = skip_manager.connect_skip(layer,cfg['filters_4'],cfg['k_4'],cfg['l2'],cfg['activation'])
        layer = Conv2D(cfg['filters_4'], (cfg['k_4'], cfg['k_4']), padding='same', kernel_regularizer=l2(cfg['l2']), bias_regularizer=l2(cfg['l2']))(layer)
        layer = Activation(cfg['activation'])(layer)
    if (cfg['stack_2']>0):
        layer = skip_manager.end_skip(layer,cfg['filters_4'],cfg['k_4'],cfg['l2'],cfg['activation'])
        if not (cfg['max_pooling']):
    ...
    for i in range(cfg['stack_3']):
        layer = skip_manager.connect_skip(layer,cfg['filters_6'],cfg['k_6'],cfg['l2'],cfg['activation'])
        layer = Conv2D(cfg['filters_6'], (cfg['k_6'], cfg['k_6']), padding='same', kernel_regularizer=l2(cfg['l2']), bias_regularizer=l2(cfg['l2']))(layer)
        layer = Activation(cfg['activation'])(layer)
    if (cfg['stack_3']>0):
        layer = skip_manager.end_skip(layer,cfg['filters_6'],cfg['k_6'],cfg['l2'],cfg['activation'])
        if not (cfg['max_pooling']):
    ...
    for i in range(cfg['stack_4']):
        layer = skip_manager.connect_skip(layer,cfg['filters_8'],cfg['k_8'],cfg['l2'],cfg['activation'])
        layer = Conv2D(cfg['filters_8'], (cfg['k_8'], cfg['k_8']), padding='same', kernel_regularizer=l2(cfg['l2']), bias_regularizer=l2(cfg['l2']))(layer)
        layer = Activation(cfg['activation'])(layer)
    if (cfg['stack_4']>0):
        layer = skip_manager.end_skip(layer,cfg['filters_8'],cfg['k_8'],cfg['l2'],cfg['activation'])
        if not (cfg['max_pooling']):
    ...
    for i in range(cfg['stack_5']):
        layer = skip_manager.connect_skip(layer,cfg['filters_10'],cfg['k_10'],cfg['l2'],cfg['activation'])
        layer = Conv2D(cfg['filters_10'], (cfg['k_10'], cfg['k_10']), padding='same', kernel_regularizer=l2(cfg['l2']), bias_regularizer=l2(cfg['l2']))(layer)
        layer = Activation(cfg['activation'])(layer)
    if (cfg['stack_5']>0):
        layer = skip_manager.end_skip(layer,cfg['filters_10'],cfg['k_10'],cfg['l2'],cfg['activation'])
        if not (cfg['max_pooling']):
    ...
    for i in range(cfg['stack_6']):
        layer = skip_manager.connect_skip(layer,cfg['filters_12'],cfg['k_12'],cfg['l2'],cfg['activation'])
        layer = Conv2D(cfg['filters_12'], (cfg['k_12'], cfg['k_12']), padding='same', kernel_regularizer=l2(cfg['l2']), bias_regularizer=l2(cfg['l2']))(layer)
        layer = Activation(cfg['activation'])(layer)
    if (cfg['stack_6']>0):
        layer = skip_manager.end_skip(layer,cfg['filters_12'],cfg['k_12'],cfg['l2'],cfg['activation'])
        if not (cfg['max_pooling']):
